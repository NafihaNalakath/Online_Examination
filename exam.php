...
//timer starts
													...
													<body>
							   						<span id="remain"></span>
									  				<form action="score.php" method="post" id="form1" name="form1">
			...
			//timer end
			//fetching questions and answers from db strat
													$t=mysqli_query($connection,"SELECT * FROM `type`");
                          $type=mysqli_fetch_array($t);
                          $typ=$type['type'];
                          $qstn=mysqli_query($connection,"SELECT `qstn_id`,`qstn` FROM `questions` where `type`='$typ'");
                          $_POST['typ']=$typ;
                          echo'<form method="POST" action="score.php">';
	                         $i=1;
                          while($row=mysqli_fetch_array($qstn) )
                          {
                          echo'<td><tr>';
                          echo $i.')  '.$row['qstn'];
                          echo '</tr></td>';
                            ...
                            echo "<br>";
                          $r=$row['qstn_id'];
												  $r1="SELECT * FROM `answers` WHERE `qstnid`='$r'";
                          $answr=mysqli_query($connection,$r1);
                          ...
                          echo'<br><br>	<div class="row form-group">
    													<div class="col-md-5">
    														<input type="submit" class="btn btn-primary btn-block" value="Submit">
    													';
                          echo'</form>';
						//fetching questions and answers from db ended
													 ?>
										</div>
									</div>
								</div>
